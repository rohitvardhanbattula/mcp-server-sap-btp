...
function isReadEntityToolDisabled() {
...
export class SAPToolRegistry {
        ...
        for (const service of this.discoveredServices) {
            ...
            for (const entityType of service.metadata.entityTypes) {
                ...
                if (entityType.creatable)
                    this.registerCreateEntityTool(service, entityType);
                if (entityType.updatable)
                    this.registerUpdateEntityTool(service, entityType);
                if (entityType.deletable)
                    this.registerDeleteEntityTool(service, entityType);
            }
        }
    }
    registerReadEntitySetTool(service, entityType) {
        const shortName = this.generateShortToolName('r', service.id, entityType.name);
        this.mcpServer.registerTool(shortName, {
            ...
            try {
                const response = await this.sapClient.readEntitySet(service.url, entityType.entitySet, queryOptions);
                return {
                    content: [{
                            type: "text",
    ...
    registerReadEntityTool(service, entityType) {
        ...
        this.mcpServer.registerTool(shortName, {
            ...
            try {
                const response = await this.sapClient.readEntity(service.url, entityType.entitySet, keyValue);
                return {
                    content: [{
                            type: "text",
                            text: JSON.stringify(response.data, null, 2)
    ...
    registerCreateEntityTool(service, entityType) {
        const createSchema = {};
        ...
        this.mcpServer.registerTool(shortName, {
            ...
            try {
                const response = await this.sapClient.createEntity(service.url, entityType.entitySet, args);
                return {
                    content: [{
                            type: "text",
                            text: JSON.stringify(response.data, null, 2)
    ...
    registerUpdateEntityTool(service, entityType) {
        const keyProperties = entityType.properties.filter(p => entityType.keys.includes(p.name));
        ...
        this.mcpServer.registerTool(shortName, {
            ...
            try {
                const response = await this.sapClient.updateEntity(service.url, entityType.entitySet, keyValue, updateData);
                return {
                    content: [{
                            type: "text",
                            text: JSON.stringify(response.data, null, 2)
    ...
    registerDeleteEntityTool(service, entityType) {
        const keyProperties = entityType.properties.filter(p => entityType.keys.includes(p.name));
        const keySchema = {};
        keyProperties.forEach(prop => {
            keySchema[prop.name] = this.getZodSchemaForODataType(prop.type).describe(`Key property: ${prop.name}`);
    ...
    buildKeyValue(keyProperties, args) {
    ...
    getZodSchemaForODataType(odataType) {
        switch (odataType) {
            case 'Edm.String':
                return z.string();
            case 'Edm.Int32':
    ...
    generateShortToolName(operation, serviceId, entityName) {
    ...
    abbreviateEntityName(entityName) {
        let abbr = entityName;
    ...
    static buildQueryOptions(args) {
        ...
        if (args.expand !== null && args.expand !== undefined && args.expand !== '') {
            queryOptions.$expand = args.expand;
        }
...
//# sourceMappingURL=sap-tool-registry.js.map